...
void vs10xx_soft_reset(void)
{
    ...
    vs10xx_spi_read_write_byte(0Xff);           /* 启动传输 */
    retry = 0;

    while (vs10xx_read_reg(SPI_MODE) != 0x0800) /*  软件复位,新模式 */
    {
        vs10xx_write_cmd(SPI_MODE, 0x0804);     /*  软件复位,新模式 */
    ...
    while (vs10xx_read_reg(SPI_CLOCKF) != 0X9800)   /* 设置VS10XX的时钟,3倍频 ,1.5xADD */
    {
        vs10xx_write_cmd(SPI_CLOCKF, 0X9800);       /* 设置VS10XX的时钟,3倍频 ,1.5xADD */
...
{
    vs10xx_reset();
    vs10xx_write_cmd(0x0b, 0X2020);     /* 设置音量 */
    vs10xx_write_cmd(SPI_MODE, 0x0820); /* 进入VS10XX的测试模式 */

    while (VS10XX_DQ == 0); /* 等待DREQ为高 */
 ...
 * @param       无
 * @retval      测试结果
 *   @arg       0X807F, VS1003测试正常
 *   @arg       0X83FF, VS1053测试正常
 *   @arg       其他,   测试失败
 */
uint16_t vs10xx_ram_test(void)
{
    ...
    delay_ms(150);
    VS10XX_XDCS(1);
    return vs10xx_read_reg(SPI_HDAT0); /*  VS1003如果得到的值为0x807F，则表明完好; VS1053为0X83FF */
}
...
{
    vs10xx_write_ram(0X1E04, speed); /* 写入播放速度 */
}
...
uint8_t vs10xx_send_music_data(uint8_t *buf)
{
    ...
    if (VS10XX_DQ != 0) /* 是否需要发送数据给 VS10XX ? */
    {
        VS10XX_XDCS(0);
...
void vs10xx_restart_play(void)
{
    ...
    temp = vs10xx_read_reg(SPI_MODE); /* 读取SPI_MODE的内容 */
    temp |= 1 << 3;                   /* 设置SM_CANCEL位 */
    temp |= 1 << 2;                   /* 设置SM_LAYER12位,允许播放MP1,MP2 */
    vs10xx_write_cmd(SPI_MODE, temp); /* 设置取消当前解码指令 */

    for (i = 0; i < 2048;) /* 发送2048个0,期间读取SM_CANCEL位.如果为0,则表示已经取消了当前解码 */
    {
        if (vs10xx_send_music_data(vsbuf) == 0) /* 每发送32个字节后检测一次 */
        {
            i += 32;                          /* 发送了32个字节 */
            temp = vs10xx_read_reg(SPI_MODE); /* 读取SPI_MODE的内容 */
    ...
    {
        vs10xx_soft_reset(); /* SM_CANCEL不成功,坏情况,需要软复位 */
    }
...
void vs10xx_reset_decode_time(void)
{
    ...
    vs10xx_write_cmd(SPI_DECODE_TIME, 0x0000); /* 操作两次 */
}
...
void vs10xx_set_volume(uint8_t volx)
{
    ...
    vs10xx_write_cmd(SPI_VOL, volt); /* 设置音量 */
}
...
void vs10xx_set_effect(uint8_t effect)
{
    ...
    vs10xx_write_cmd(SPI_MODE, temp); /* 设定模式 */
}
...
{
    vs10xx_write_ram(GPIO_DDR, 1 << 4);    /* VS1053的GPIO4设置成输出 */
    vs10xx_write_ram(GPIO_ODATA, sw << 4); /* 控制VS1053的GPIO4输出值(0/1) */
}
...
void vs10xx_set_all(void)
{
    ...
    vs10xx_set_tone(vsset.bflimit, vsset.bass, vsset.tflimit, vsset.treble); /* 高低音设置 */
    vs10xx_set_effect(vsset.effect);                                         /* 设置空间效果 */
    vs10xx_set_speaker(vsset.speakersw);                                     /* 控制板载喇叭状态 */
}
